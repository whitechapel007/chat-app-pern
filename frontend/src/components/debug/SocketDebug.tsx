    ...
    <div className="fixed bottom-4 right-4 bg-base-100 border border-base-300 rounded-lg p-4 shadow-lg max-w-sm z-50">
      ...
      <div className="space-y-2 text-sm">
        <div>
          ...
          <span
            className={`ml-2 ${isConnected ? "text-success" : "text-error"}`}
          >
            {connectionStatus}
          </span>
        </div>
          ...
          {onlineUsers.length === 0 ? (
            ...
            <div className="mt-1 space-y-1">
              {onlineUsers.map((user, index) => (
                <div
                  key={user.userId}
                  className="text-xs bg-base-200 p-1 rounded"
                >
                  <div>
                    <strong>#{index + 1}</strong>
                  </div>
                  <div>
                    <strong>ID:</strong> {user.userId.slice(0, 8)}...
                  </div>
                  <div>
                    <strong>Socket:</strong> {user.socketId?.slice(0, 8)}...
                  </div>
                  <div>
                    <strong>Online:</strong> {user.isOnline ? "✅" : "❌"}
                  </div>
                </div>
              ))}
            </div>
