...
import { getOnlineUsers, getUserSocketId, isUserOnline, } from "../socket/index.js";
...
export const updateOnlineStatus = asyncHandler(async (req, res, next) => {
    ...
    const updatedUser = await userService.updateUserOnlineStatus(userId, isOnline);
    res
        .status(200)
        .json(formatSuccessResponse(updatedUser, "Online status updated successfully"));
});
// Get user statistics
export const getUserStats = asyncHandler(async (req, res, next) => {
    ...
    res
        .status(200)
        .json(formatSuccessResponse(stats, "User statistics retrieved successfully"));
});
// Search users
export const searchUsers = asyncHandler(async (req, res, next) => {
    ...
    const users = await userService.searchUsers(query, excludeUserId, searchLimit);
    res
        .status(200)
...
export const getUsersForConversation = asyncHandler(async (req, res, next) => {
    if (!req.user) {
        return res.status(401).json({
            success: false,
            error: { message: "Not authenticated", statusCode: 401 },
    ...
    res
        .status(200)
        .json(formatSuccessResponse(result.users, "Users for conversation retrieved successfully"));
});
export const getUsersOnline = asyncHandler(async (req, res) => {
