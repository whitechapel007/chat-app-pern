...
export const createGroupConversation = async (creatorId, conversationData) => {
    ...
    await prisma.message.create({
        data: {
            content: `${users.find((u) => u.id === creatorId)?.fullName || "Someone"} created the group "${name}"`,
            type: "SYSTEM",
            senderId: creatorId,
...
export const addParticipantToGroup = async (conversationId, userId, addedByUserId) => {
    return await prisma.$transaction(async (tx) => {
        ...
        await tx.message.create({
            data: {
                content: `${adder?.fullName || "Someone"} added ${user.fullName || user.username} to the group`,
                type: "SYSTEM",
...
export const removeParticipantFromGroup = async (conversationId, userId, removedByUserId) => {
    return await prisma.$transaction(async (tx) => {
        // Check if conversation exists and is a group
        const conversation = await tx.conversation.findUnique({
            where: { id: conversationId },
        ...
        if (conversation.type !== "GROUP") {
            throw new ValidationError("Can only remove participants from group conversations");
        }
        ...
        const systemMessage = isSelfRemoval
            ? `${participant.user.fullName || participant.user.username} left the group`
            : `${remover?.fullName || "Someone"} removed ${participant.user.fullName || participant.user.username} from the group`;
        await tx.message.create({
            data: {
...
export const updateGroupConversation = async (conversationId, userId, updates) => {
    return await prisma.$transaction(async (tx) => {
        ...
        if (updates.name) {
            await tx.message.create({
                data: {
                    content: `${user?.fullName || "Someone"} changed the group name to "${updates.name}"`,
                    type: "SYSTEM",
                    senderId: userId,
                    conversationId,
                },
...
export const sendMessage = async (senderId, conversationId, data) => {
    // Validate input data
    const { value: validatedData, error } = validateInput(messageSchema, data);
    if (error) {
...
export const sendDirectMessage = async (senderId, receiverId, data) => {
